...
bFit <- glmer(W ~ x1+(1|IDFac), data = dat[dat$x1 == 1,],family = binomial(link= "probit"))
summary(bFit)
...
Q[2] <- 100#sum.finite(log(pi1)+log(dnorm(x, mu1, sigma1))) + sum.finite(log(pi2)+log(dnorm(x, mu2, sigma2)))
